/*==============================================================================
Generated on: 2023-12-28 01:59:36.517486
Generated by: _convert.py in this directory
Source: https://github.com/betaflight/unified-targets
Board name: DALRCF722DUAL
Manufacturer ID: DALR

//copy this line to hw_STM32.h to use this flight controller, or copy/paste this file into hw_STM32.h
#include "betaflight-targets/DALR-DALRCF722DUAL.config.h"
==============================================================================*/
#define USE_IMU_SPI_MPU6500
#define USE_IMU_SPI_MPU6000
#define USE_BARO_MS5611
#define USE_FLASH_W25Q128FV
#define USE_MAX7456

//LED:
const int HW_PIN_LED      = PC14;
#define LED_ON 0 //low = on

//Battery voltage divider:
const int HW_PIN_BAT_ADC  = PC1;
const int HW_PIN_BAT_CURR = PC0;

//GPS: (SERIAL1)
const int HW_PIN_GPS_RX   = PB7;
const int HW_PIN_GPS_TX   = PB6;

//RC Receiver: (SERIAL3)
const int HW_PIN_RCIN_RX  = PC11;
const int HW_PIN_RCIN_TX  = PC10;

//IMU:
const int HW_PIN_IMU_INT  = PB10;

//I2C: (I2C1)
const int HW_PIN_I2C_SDA  = PB8;
const int HW_PIN_I2C_SCL  = PB9;
typedef TwoWire HW_WIRETYPE; //define the class to use for I2C

//SPI: (SPI1)
const int HW_PIN_SPI_MISO = PA6;
const int HW_PIN_SPI_CS   = PB0;
const int HW_PIN_SPI_SCLK = PA5;
const int HW_PIN_SPI_MOSI = PA7;

//Outputs:
#define HW_OUT_COUNT 6
const int16_t HW_PIN_OUT[HW_OUT_COUNT] = {PC6,PC7,PC8,PC9,PA8,PA9};


/*
#==============================================================================
# BetaFlight Source file
#==============================================================================
# Betaflight / STM32F7X2 (S7X2) 4.1.0 Oct 16 2019 / 11:58:45 (c37a7c91a) MSP API: 1.42

#define USE_ACC
#define USE_ACC_SPI_MPU6500
#define USE_ACC_SPI_MPU6000
#define USE_GYRO
#define USE_GYRO_SPI_MPU6500
#define USE_GYRO_SPI_MPU6000
#define USE_BARO
#define USE_BARO_MS5611
#define USE_FLASH
#define USE_FLASH_W25Q128FV
#define USE_MAX7456

board_name DALRCF722DUAL
manufacturer_id DALR

# resources
resource BEEPER 1 C13
resource MOTOR 1 C06
resource MOTOR 2 C07
resource MOTOR 3 C08
resource MOTOR 4 C09
resource MOTOR 5 A08
resource MOTOR 6 A09
resource PPM 1 B07
resource LED_STRIP 1 A15
resource SERIAL_TX 1 B06
resource SERIAL_TX 2 A02
resource SERIAL_TX 3 C10
resource SERIAL_TX 4 A00
resource SERIAL_TX 5 C12
resource SERIAL_RX 1 B07
resource SERIAL_RX 2 A03
resource SERIAL_RX 3 C11
resource SERIAL_RX 4 A01
resource SERIAL_RX 5 D02
resource I2C_SCL 1 B08
resource I2C_SDA 1 B09
resource LED 1 C14
resource SPI_SCK 1 A05
resource SPI_SCK 2 B13
resource SPI_SCK 3 B03
resource SPI_MISO 1 A06
resource SPI_MISO 2 B14
resource SPI_MISO 3 B04
resource SPI_MOSI 1 A07
resource SPI_MOSI 2 B15
resource SPI_MOSI 3 B05
resource CAMERA_CONTROL 1 B01
resource ADC_BATT 1 C01
resource ADC_RSSI 1 A00
resource ADC_CURR 1 C00
resource FLASH_CS 1 B02
resource OSD_CS 1 B12
resource GYRO_EXTI 1 B10
resource GYRO_EXTI 2 C04
resource GYRO_CS 1 B00
resource GYRO_CS 2 A04

# timer
timer B07 AF2
# pin B07: TIM4 CH2 (AF2)
timer C06 AF3
# pin C06: TIM8 CH1 (AF3)
timer C07 AF3
# pin C07: TIM8 CH2 (AF3)
timer C08 AF3
# pin C08: TIM8 CH3 (AF3)
timer C09 AF3
# pin C09: TIM8 CH4 (AF3)
timer A08 AF1
# pin A08: TIM1 CH1 (AF1)
timer A09 AF1
# pin A09: TIM1 CH2 (AF1)
timer A15 AF1
# pin A15: TIM2 CH1 (AF1)
timer B01 AF2
# pin B01: TIM3 CH4 (AF2)

# dma
dma ADC 3 1
# ADC 3: DMA2 Stream 1 Channel 2
dma pin B07 0
# pin B07: DMA1 Stream 3 Channel 2
dma pin C06 0
# pin C06: DMA2 Stream 2 Channel 0
dma pin C07 0
# pin C07: DMA2 Stream 2 Channel 0
dma pin C08 0
# pin C08: DMA2 Stream 2 Channel 0
dma pin C09 0
# pin C09: DMA2 Stream 7 Channel 7
dma pin A08 0
# pin A08: DMA2 Stream 6 Channel 0
dma pin A09 0
# pin A09: DMA2 Stream 6 Channel 0
dma pin A15 0
# pin A15: DMA1 Stream 5 Channel 3
dma pin B01 0
# pin B01: DMA1 Stream 2 Channel 5

# feature
feature RX_SERIAL
feature OSD

# serial
serial 0 64 115200 57600 0 115200

# master
set mag_bustype = I2C
set mag_i2c_device = 1
set baro_bustype = I2C
set baro_i2c_device = 1
set serialrx_provider = SBUS
set adc_device = 3
set blackbox_device = SPIFLASH
set dshot_burst = ON
set current_meter = ADC
set battery_meter = ADC
set vbat_scale = 160
set ibata_scale = 166
set beeper_inversion = ON
set beeper_od = OFF
set max7456_spi_bus = 2
set flash_spi_bus = 3
set gyro_1_bustype = SPI
set gyro_1_spibus = 1
set gyro_1_sensor_align = CW180
set gyro_1_align_yaw = 1800
set gyro_2_spibus = 1

*/

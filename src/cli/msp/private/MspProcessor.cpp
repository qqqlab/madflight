...
//OK:   MSP_BOARD_INFO                  4    // out message: Get board information
//OK:   MSP_STATUS                      101  // out message: Cycletime & errors_count & sensor present & box activation & 
//OK:   MSP_UID                         160  // out message: Unique device ID
//TODO: MSP2_GET_TEXT                       0x3006, MSP2TEXT_BUILDKEY                        5
//TODO: MSP2_GET_TEXT                       0x3006, MSP2TEXT_CRAFT_NAME                      2
//TODO: MSP2_GET_TEXT                       0x3006, MSP2TEXT_PILOT_NAME                      1
//OK:   MSP_SET_ARMING_DISABLED         99   // in message:  Enable/disable arming
//TODO: MSP_SET_RTC                     246  // in message:  Set the RTC clock
//TODO: MSP_CURRENT_METER_CONFIG        40   // out message: Get current meter configuration
//TODO: MSP_BATTERY_CONFIG              32   // out message: Get battery configuration
//TODO: MSP_BLACKBOX_CONFIG             80   // out message: Get blackbox settings
//TODO: MSP_DATAFLASH_SUMMARY           70   // out message: Get description of dataflash chip
//TODO: MSP_SDCARD_SUMMARY              79   // out message: Get SD card state
//OK:   MSP_ACC_TRIM                    240  // out message: Get acc angle trim values
//OK:   MSP_STATUS_EX                   150  // out message: Cycletime, errors_count, CPU load, sensor present etc
//TODO: MSP2_MCU_INFO                       0x300C
//OK:   MSP_MIXER_CONFIG                42   // out message: Get mixer configuration
//OK:   MSP_SENSOR_ALIGNMENT            126  // out message: Orientation of acc,gyro,mag
//TODO: MSP2_SENSOR_CONFIG_ACTIVE           0x300A

loop:
//OK:   MSP_ATTITUDE                    108  // out message: 2 angles 1 heading
//OK:   MSP_ANALOG                      110  // out message: Vbat, powermetersum, rssi if available on RX
//OK:   MSP_BATTERY_STATE               130  // out message: Connected/Disconnected, Voltage, Current Used
//OK:   MSP_STATUS_EX                   150  // out message: Cycletime, errors_count, CPU load, sensor present etc
*/

//shown as "manufacturerName/boardName(targetName)" in status bar and connection log
//if boardName is empty then boardIdentifier is shown as boardName
//if manufacturerName and targetName are empty then boardIdentifier is shown as targetName
const char * const manufacturerName = "";
const char * const boardName = ""; 
const char * const boardIdentifier = "MADF"; //4 UPPER CASE alpha numeric characters
const char * const targetName = MADFLIGHT_VERSION;

const char * const targetVersion = "0000000";

#define GIT_SHORT_REVISION_LENGTH   7 // lower case hexadecimal digits.
const char * const shortGitRevision = "v0.0.0";

//date and time shown in configurator log
#define BUILD_TIME_LENGTH 8
const char * const buildTime = __TIME__;  // "HH:MM:SS"

#define BUILD_DATE_LENGTH 11
const char * const buildDate = __DATE__; // "MMM DD YYYY" MMM = Jan/Feb/...

const char * flightControllerIdentifier = "BTFL"; //needs to be BTFL for app.betaflight.com

//can be anything, but some functions (servo) appear to check version
...
#define getBoardId0 0xDEADBEEF
#define getBoardId1 0xDEADBEEF
#define getBoardId2 0xDEADBEEF

enum MixerType {
  FC_MIXER_TRI = 1,
  FC_MIXER_QUADP = 2,  //shown as X
  FC_MIXER_QUADX = 3,  //nice
  FC_MIXER_BICOPTER = 4,
  FC_MIXER_GIMBAL = 5,
  FC_MIXER_Y6 = 6,   //nice
  FC_MIXER_HEX6 = 7,   //nice
  FC_MIXER_FLYING_WING = 8, //ugly wing
  FC_MIXER_Y4 = 9,
  FC_MIXER_HEX6X = 10,
  FC_MIXER_OCTOX8 = 11,
  FC_MIXER_OCTOFLATP = 12,
  FC_MIXER_OCTOFLATX = 13,
  FC_MIXER_AIRPLANE = 14,        //shown as biplane in configurator
  FC_MIXER_HELI_120_CCPM = 15,  //ugly wing
  FC_MIXER_HELI_90_DEG = 16,
  FC_MIXER_VTAIL4 = 17,
  FC_MIXER_HEX6H = 18,  //ugly wing
  FC_MIXER_PPM_TO_SERVO = 19,    // PPM -> servo relay
  FC_MIXER_DUALCOPTER = 20,
  FC_MIXER_SINGLECOPTER = 21, //ugly wing
  FC_MIXER_ATAIL4 = 22,
  FC_MIXER_CUSTOM = 23,
  FC_MIXER_CUSTOM_AIRPLANE = 24,  //ugly wing
  FC_MIXER_CUSTOM_TRI = 25,
  FC_MIXER_QUADX_1234 = 26,
};

#define FC_MIXERRTYPE FC_MIXER_QUADX

#define VTXCOMMON_MSP_BANDCHAN_CHKVAL ((uint16_t)((7 << 3) + 7))

extern "C" {
  #include "betaflight/msp_protocol.h"
  #include "betaflight/msp_protocol_v2_common.h"
  #include "betaflight/msp_protocol_v2_betaflight.h"
  //#include "betaflight/serial_4way.h"
...
{
  if(speed >= 2470000) return SERIAL_SPEED_INDEX_2470000;
  if(speed >= 2000000) return SERIAL_SPEED_INDEX_2000000;
  if(speed >= 1500000) return SERIAL_SPEED_INDEX_1500000;
  if(speed >= 1000000) return SERIAL_SPEED_INDEX_1000000;
  if(speed >= 921600)  return SERIAL_SPEED_INDEX_921600;
  if(speed >= 500000)  return SERIAL_SPEED_INDEX_500000;
  if(speed >= 460800)  return SERIAL_SPEED_INDEX_460800;
  if(speed >= 400000)  return SERIAL_SPEED_INDEX_400000;
  if(speed >= 250000)  return SERIAL_SPEED_INDEX_250000;
  if(speed >= 230400)  return SERIAL_SPEED_INDEX_230400;
  if(speed >= 115200)  return SERIAL_SPEED_INDEX_115200;
  if(speed >= 57600)   return SERIAL_SPEED_INDEX_57600;
  if(speed >= 38400)   return SERIAL_SPEED_INDEX_38400;
  if(speed >= 19200)   return SERIAL_SPEED_INDEX_19200;
  if(speed >= 9600)    return SERIAL_SPEED_INDEX_9600;
  return SERIAL_SPEED_INDEX_AUTO;
}

static int32_t fromBaudIndex(SerialSpeedIndex index)
{
  switch(index)
  {
    case SERIAL_SPEED_INDEX_9600:    return 9600;
    case SERIAL_SPEED_INDEX_19200:   return 19200;
    case SERIAL_SPEED_INDEX_38400:   return 38400;
    case SERIAL_SPEED_INDEX_57600:   return 57600;
    case SERIAL_SPEED_INDEX_115200:  return 115200;
    case SERIAL_SPEED_INDEX_230400:  return 230400;
    case SERIAL_SPEED_INDEX_250000:  return 250000;
    case SERIAL_SPEED_INDEX_400000:  return 400000;
    case SERIAL_SPEED_INDEX_460800:  return 460800;
    case SERIAL_SPEED_INDEX_500000:  return 500000;
    case SERIAL_SPEED_INDEX_921600:  return 921600;
    case SERIAL_SPEED_INDEX_1000000: return 1000000;
    case SERIAL_SPEED_INDEX_1500000: return 1500000;
    case SERIAL_SPEED_INDEX_2000000: return 2000000;
    case SERIAL_SPEED_INDEX_2470000: return 2470000;
...
{
  switch(t) {
    case 0: return FILTER_NONE;
    case 1: return FILTER_PT3;
    case 2: return FILTER_BIQUAD;
    default: return FILTER_PT3;
  }
}
...
static uint8_t fromFilterTypeDerivative(uint8_t t)
{
  switch(t) {
    ...
    case FILTER_PT3: return 1;
    case FILTER_BIQUAD: return 2;
    default: return 1;
  }
}
...
{
  switch(t) {
    case GYRO_DLPF_256: return 0;
    case GYRO_DLPF_EX: return 1;
    default: return 2;
  }
}
...
void MspProcessor::processCommand(MspMessage& m, MspResponse& r)
{
  r.cmd = m.cmd;
  r.version = m.version;
  r.result = 1;
  switch(m.cmd)
  {
    ...
    case MSP_BOARD_INFO: //4 0x04
      ...
      r.writeU8(strlen(boardName));  // board name
      r.writeData(boardName, strlen(boardName));
      r.writeU8(strlen(manufacturerName));   // manufacturer name
      r.writeData( manufacturerName, strlen( manufacturerName));
      for(size_t i = 0; i < 32; i++) r.writeU8(0); // signature
      r.writeU8(255); // mcu id: unknown
      // 1.42
      r.writeU8(2);  // configuration state: configured
      // 1.43
      r.writeU16(1000); // sample rate //_model.state.gyro.present ? _model.state.gyro.timer.rate : 0); // sample rate
      {
        uint32_t problems = 0;
    ...
    case MSP_STATUS:     //101 0x65 -> E7 10 00 00 23 00 00 00 00 00 00 0C 00 00 00 00 1A 04 10 10 00 00 29 00
      ...
      r.writeU8(0); //15 count

      // Write arming disable flags
      r.writeU8(0); //16 ARMING_DISABLED_FLAGS_COUNT);  // 1 byte, flag count
...
//        if (_model.isModeActive(MODE_ARMED)) _model.disarm(DISARM_REASON_ARMING_DISABLED);
    ...
    case MSP_ATTITUDE: //108 0x6C
    {
      ...
      r.writeU16(lrintf(ahr.roll * 10));// roll  [decidegrees]
      r.writeU16(lrintf(-ahr.pitch * 10));// // pitch [decidegrees]
      r.writeU16(lrintf(ahr.yaw)); // yaw   [degrees]
      break;
    }
    ...
    case MSP_SENSOR_ALIGNMENT: //126 0x7E
      ...
      //1.41+
      r.writeU8(1);//_model.state.gyro.present ? 1 : 0); // gyro detection mask GYRO_1_MASK
      r.writeU8(0); // gyro_to_use
      r.writeU8(3);//_model.config.gyro.align); // gyro 1
      r.writeU8(0); // gyro 2
      break;
    ...
    case MSP_BATTERY_STATE: //130 0x82
      ...
      r.writeU16(lrintf(bat.mah)); // milliamp hours drawn from battery
      r.writeU16(lrintf(bat.i * 100));//toIbatCurrent(_model.state.battery.current)); // current in 0.01A
      r.writeU8(0); // battery alerts ???
    ...
    case MSP_ANALOG: //110 0x6E
      ...
      r.writeU16(lrintf(bat.i * 100)); // current in 0.01A
...
void MspProcessor::sendResponse(MspResponse& r, MF_Serial* s)
{
  //debugResponse(r);
  uint8_t buff[256];
